"""
Prompt templates for the Chart Maker Agent in Alex Financial Advisor.

This module defines:

* The core instruction prompt (`CHARTER_INSTRUCTIONS`) that constrains the
  Chart Maker Agent to return **JSON-only** chart specifications for an
  investment portfolio.
* A helper function (`create_charter_task`) that wraps the analytical
  summary of a portfolio into a concise task prompt for the agent.

The goal is to ensure that downstream chart generation is:

* Structured — charts follow a consistent JSON schema.
* Focused — the LLM is guided by a human-readable portfolio analysis rather
  than raw, verbose data.
* Safe — the agent is explicitly forbidden from emitting any non-JSON text.
"""

import json  # noqa: F401  # Reserved for potential future JSON serialisation use
...
# Instruction block used to prime the Chart Maker Agent to output JSON-only chart specs
CHARTER_INSTRUCTIONS: str = """You are a Chart Maker Agent that creates visualization data for investment portfolios.
...
def create_charter_task(
    portfolio_analysis: str,
    portfolio_data: Dict[str, Any],
) -> str:
    """
    Build the task prompt for the Chart Maker Agent.

    This function takes a **pre-computed textual analysis** of the portfolio
    (for example, from a prior aggregation step) and wraps it in concise
    instructions that tell the agent to produce 4–6 charts in JSON format.

    The raw `portfolio_data` is intentionally **not** inlined into the prompt
    in order to keep the context compact. The analysis text is assumed to
    already reflect all relevant aggregation and interpretation.

    Parameters
    ----------
    portfolio_analysis : str
        Human-readable summary of portfolio metrics (allocations, top holdings,
        account breakdowns, etc.) generated by earlier processing.
    portfolio_data : dict of str to Any
        Original portfolio payload. Included here for future flexibility or
        debugging, but not embedded in the returned prompt string.

    Returns
    -------
    str
        A complete task prompt instructing the Chart Maker Agent to construct
        4–6 JSON-only chart specifications based on the provided analysis.
    """
    # Start the task prompt with high-level instructions for the agent
    header = "Analyze this investment portfolio and create 4-6 visualization charts.\n"

    # Insert the detailed portfolio analysis as contextual grounding
    body = f"\n{portfolio_analysis}\n\n"

    # Conclude with a strict reminder about JSON-only output
    footer = (
        "Create charts based on this portfolio data. "
        "Calculate aggregated values from the positions shown above.\n\n"
        "OUTPUT ONLY THE JSON OBJECT with 4-6 charts - no other text."
    )

    # Concatenate header, analysis body, and footer into the final prompt
